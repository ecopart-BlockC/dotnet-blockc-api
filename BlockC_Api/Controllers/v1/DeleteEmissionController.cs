...
namespace BlockC_Api.Controllers.v1
{
    ...
    [ApiVersion("1.0")]
    [System.Web.Http.Route("api/v{version:apiVersion}/DeleteEmission")]
    public class DeleteEmissionController : ApiController
    {
        public async Task<HttpResponseMessage> Post([System.Web.Http.FromBody] JObject _request)
        {
            ...
            try
            {
                Classes.Json.DeleteEmissionRequest emissionRequest = new Classes.Json.DeleteEmissionRequest();
                emissionRequest = JsonConvert.DeserializeObject<Classes.Json.DeleteEmissionRequest>(_request.ToString());
                ...
                if (string.IsNullOrEmpty(emissionRequest.EmissaoID))
                {
                    genericResponse.mensagem = "Necessário informar emissionId e userId";
                    jsonResponse = JsonConvert.SerializeObject(genericResponse).ToString();
                    response = Request.CreateResponse(System.Net.HttpStatusCode.BadRequest);
                    response.Content = new StringContent(jsonResponse, Encoding.UTF8, "application/json");
                    return response;
                ...
                if (!database.DesativarFatorEmissao(emissionRequest.EmissaoID, emissionRequest.UsuarioID))
                {
